    ...
    return (
        <header
            className={cn(
                ...
                isScrolled ? "bg-[#0b1624]/80 backdrop-blur-xl border-white/10 shadow-2xl py-3" : "bg-transparent py-5"
            )}
                ...
                <div className="flex items-center justify-between relative">
                    ...
                    <div className="hidden lg:flex items-center gap-3">
                        <form
                            ...
                            className={cn(
                                "flex items-center bg-white/5 border border-white/10 rounded-full px-3 py-2 transition-all duration-300 hover:border-primary/30",
                                searchOpen ? "w-64 bg-[#0b1624] border-primary/50 shadow-lg" : "w-10 cursor-pointer hover:bg-white/10"
                            )}>
                            <Search
                                className={cn("w-4 h-4 shrink-0 transition-colors", searchOpen ? "text-primary" : "text-muted-foreground")}
                                onClick={() => {
                        ...
                        <Button size="sm" className="bg-primary hover:bg-primary/90 text-white shadow-lg shadow-primary/20 rounded-full px-6 font-bold">
                            Subscribe
                        </Button>
                    </div>
                            ...
                            <div className="grid grid-cols-12 gap-8">
                                ...
                                <div className="col-span-3 border-r border-white/5 pr-8">
                                    ...
                                    <ul className="space-y-2">
                                        {navLinks.find(l => l.name === activeDropdown)?.subcategories?.map((sub) => (
                                            <li key={sub}>
                                                <Link
                                ...
                                <div className="col-span-6">
                                    ...
                                    {(() => {
                                        ...
                                        return link?.featured ? (
                                            <div className="group cursor-pointer relative h-48 rounded-xl overflow-hidden border border-white/10">
                                ...
                                <div className="col-span-3 pl-8 border-l border-white/5">
                                    <div className="bg-primary/5 border border-primary/10 rounded-xl p-4 text-center">
                                        ...
                                        <Button size="sm" variant="outline" className="w-full border-primary/20 hover:bg-primary hover:text-white">
                                            Start Free Trial
                                        </Button>
                                    </div>
                                </div>
                        ...
                        <nav className="flex flex-col p-4 gap-2">
                            {navLinks.map((link) => (
                                <Link
                                    key={link.name}
                                    href={link.href}
                                    className="text-lg font-bold text-muted-foreground hover:text-primary py-3 border-b border-white/5 last:border-0"
                            ...
                            <div className="flex flex-col gap-3 mt-6">
                                ...
                                <Button className="w-full justify-center h-12 bg-primary text-white">Subscribe Now</Button>
                            </div>
                        </nav>
                    </motion.div>
                )}
